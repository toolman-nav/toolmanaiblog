...
## 自动补发与验证
保留北京时间08:00检查；另在每小时13分、43分补查所有已到期非草稿文章。只有文章返回404才触发构建；已上线则跳过，网络错误、重定向或服务器错误会报告失败，不误判为缺文章。触发后最多检查20轮，每轮间隔30秒，验证HTTP200及正确canonical。失败会显示Actions错误与摘要；邮件需要在GitHub通知设置中开启Actions失败通知。所有检查仍依赖GitHub调度，不能保证准点。
